...
class EELSModel(nn.Module):
    ...
    def __init__(
        ...
        # input channels: x, mask, coord, dx, d2x
        self.stem1 = ConvBlock(5, 64, 7, p=dropout)
        ...
        # per-class evidence map
        self.class_map_head = nn.Conv1d(hidden, num_classes, kernel_size=1)

        # per-class attention scores
        # input = concat(hidden features, class evidence) -> [B, hidden + num_classes, N]
        self.attn_head = nn.Conv1d(hidden + num_classes, num_classes, kernel_size=1)

        # class-wise linear combiner over [att, max, mean]
    ...
    def init_bias_from_prevalence(self, prevalence: torch.Tensor) -> None:
        """
        prevalence: [K] class frequencies.
        Initializes bias so that sigmoid(bias) ≈ prevalence.
        """
        p = prevalence.clamp(1e-4, 1.0 - 1e-4)
        bias = torch.log(p / (1.0 - p))
        with torch.no_grad():
            self.logit_bias.copy_(bias)
    ...
    def _build_input_channels(
        ...
        d2x[..., 1:-1] = x[..., 2:] - 2.0 * x[..., 1:-1] + x[..., :-2]

        inp = torch.cat([x, mask.to(dtype), coord, dx, d2x], dim=1)
        return inp
    ...
    def forward(
        ...
        # evidence map per class
        class_map = self.class_map_head(h)  # [B, K, N]

        # attention depends on both features and evidence
        attn_inp = torch.cat([h, class_map], dim=1)  # [B, hidden + K, N]
        attn_scores = self.attn_head(attn_inp)       # [B, K, N]
        attention = masked_softmax(attn_scores, mask, dim=-1)  # [B, K, N]
        ...
        # pooled summaries per class
        pooled_att = (class_map * attention).sum(dim=-1)         # [B, K]
        pooled_max = masked_max(class_map, class_mask, dim=-1)   # [B, K]
        pooled_mean = masked_mean(class_map, class_mask, dim=-1) # [B, K]

        pooled = torch.stack([pooled_att, pooled_max, pooled_mean], dim=-1)  # [B, K, 3]
