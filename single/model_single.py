...
class EELSSingleModel(nn.Module):
    """
    Single-element classifier for EELS spectra.
    ...
    def __init__(self, num_classes: int = 80, length: int = 3072, hidden: int = 128, dropout: float = 0.10):
        super().__init__()
        ...
        # per-class attention scores for attention pooling
        self.attn_head = nn.Conv1d(hidden, num_classes, kernel_size=1)

        # class-wise affine combiner:
        # for each class combine [att_pool, max_pool, mean_pool]
        self.logit_scale = nn.Parameter(torch.ones(num_classes, 3))
        self.logit_bias = nn.Parameter(torch.zeros(num_classes))

        num_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"EELSSingleModel initialized with {num_params} trainable parameters.")
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> dict:
        ...
        # class-specific attention over energy
